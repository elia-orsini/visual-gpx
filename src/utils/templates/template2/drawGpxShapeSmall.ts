...
export function drawGpxShapeSmall(image: any, points: any) {
  ...
  const project = (lat: number, lon: number) => {
    const paddedMinLon = minLon - (maxLon - minLon) * 0.5 ;
    const paddedMaxLon = maxLon + (maxLon - minLon) * 0.5 ;
    const paddedMinLat = minLat - (maxLat - minLat) * 0.5 ;
    const paddedMaxLat = maxLat + (maxLat - minLat) * 0.5 ;
